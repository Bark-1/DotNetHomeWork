...
namespace OrderWinForm
{
    public partial class Form1 : Form
    {
        ...
        {
            if (DeleteRow >= Os.OrderList.Count) return;
            Os.DelOrder(Os.OrderList[DeleteRow].OrderID);
            bindingSourceAllOrder.ResetBindings(false);
        }
        ...
        {
            bindingSourceFindOrder.DataSource = Os.FindOrder(OrderNumber).ToList<Order>();
            bindingSourceFindOrder.ResetBindings(false);
        }
        private void buttonFindPrice_Click(object sender, EventArgs e)
        {
            bindingSourceFindOrder.DataSource = Os.FindOrder(Price).ToList<Order>();
            bindingSourceFindOrder.ResetBindings(false);
        }
        private void buttonFindGoods_Click(object sender, EventArgs e)
        {
            bindingSourceFindOrder.DataSource = Os.FindOrder(GoodsSelected.GoodsName).ToList<Order>();
            bindingSourceFindOrder.ResetBindings(false);
        }
        private void buttonFindCus_Click(object sender, EventArgs e)
        {
            bindingSourceFindOrder.DataSource = Os.FindOrder(CustomerSelected).ToList<Order>();
            bindingSourceFindOrder.ResetBindings(false);
        }
        ...
        private void dataGridViewAll_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            labelMessage.Text = "Choose row " + DeleteRow.ToString();
        }
    }
}
